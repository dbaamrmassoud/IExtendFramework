...
namespace IExtendFramework.Text
{
    public class INIDocument
    {
        ...
        public void Load(string filename, bool emptyOldData = true)
        {
            if (emptyOldData)
                data.Clear();
            ReadINIData(new FileStream(filename, FileMode.Open), Encoding.UTF8);
        }
        
        public void Load(Stream stream, bool emptyOldData = true)
        {
            if (emptyOldData)
                data.Clear();
            ReadINIData(stream, Encoding.UTF8);
        }
    }
}
