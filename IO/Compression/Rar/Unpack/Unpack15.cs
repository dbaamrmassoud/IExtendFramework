...
using System;
using System.IO;
using BitInput = IExtendFramework.IO.Compression.Rar.Unpack.VM.BitInput;
...
namespace IExtendFramework.IO.Compression.Rar.Unpack
{
    ...
    internal abstract class Unpack15 : BitInput
    {
        ...
        protected internal int[] ChSet = new int[256], ChSetA = new int[256], ChSetB = new int[256], ChSetC = new int[256];

        protected internal int[] Place = new int[256], PlaceA = new int[256], PlaceB = new int[256], PlaceC = new int[256];
        ...
        internal static int[] ShortXor1 = new int[] { 0, 0xa0, 0xd0, 0xe0, 0xf0, 0xf8, 0xfc, 0xfe, 0xff, 0xc0, 0x80, 0x90, 0x98, 0x9c, 0xb0 };

        internal static int[] ShortLen2 = new int[] { 2, 3, 3, 3, 4, 4, 5, 6, 6, 4, 4, 5, 6, 6, 4, 0 };

        internal static int[] ShortXor2 = new int[] { 0, 0x40, 0x60, 0xa0, 0xd0, 0xe0, 0xf0, 0xf8, 0xfc, 0xc0, 0x80, 0x90, 0x98, 0x9c, 0xb0 };
        ...
        private void shortLZ()
        {
            ...
            BitField = Utilities.URShift(BitField, 8);
            if (AvrLn1 < 37)
            {
                for (Length = 0; ; Length++)
                {
                    if (((BitField ^ ShortXor1[Length]) & (~(Utilities.URShift(0xff, getShortLen1(Length))))) == 0)
                    {
                        break;
                    }
                }
        ...
        private void longLZ()
        {
            ...
            AvrLn2 += Length;
            AvrLn2 -= Utilities.URShift(AvrLn2, 5);
            ...
            Distance = Utilities.URShift(((Distance & 0xff00) | (Utilities.URShift(GetBits(), 8))), 1);
            AddBits(7);
        ...
        private void huffDecode()
        {
            ...
            if (StMode != 0)
            {
                ...
                if (--BytePlace == -1)
                {
                    ...
                    else
                    {
                        ...
                        Distance = (Distance << 5) | (Utilities.URShift(GetBits(), 11));
                        AddBits(5);
                        oldCopyString(Distance, Length);
                        return;
                    }
            ...
            AvrPlc += BytePlace;
            AvrPlc -= Utilities.URShift(AvrPlc, 8);
            Nhfb += 16;
            if (Nhfb > 0xff)
            {
                Nhfb = 0x90;
                Nlzb = Utilities.URShift(Nlzb, 1);
            }

            window[unpPtr++] = (byte)(Utilities.URShift(ChSet[BytePlace], 8));
            --destUnpSize;
        ...
        private void getFlagsBuf()
        {
            ...
            while (true)
            {
                ...
                FlagBuf = Utilities.URShift(Flags, 8);
        ...
        private void initHuff()
        {
            ...
            Utilities.Fill(NToPl, 0); // memset(NToPl,0,sizeof(NToPl));
            Utilities.Fill(NToPlB, 0); // memset(NToPlB,0,sizeof(NToPlB));
            Utilities.Fill(NToPlC, 0); // memset(NToPlC,0,sizeof(NToPlC));
            corrHuff(ChSetB, NToPlB);
        }
        ...
        private void corrHuff(int[] CharSet, int[] NumToPlace)
        {
            ...
            Utilities.Fill(NumToPlace, 0); // memset(NumToPlace,0,sizeof(NToPl));
            for (I = 6; I >= 0; I--)
            {
                NumToPlace[I] = (7 - I) * 32;
            }
        ...
        private int decodeNum(int Num, int StartPos, int[] DecTab, int[] PosTab)
        {
            ...
            AddBits(StartPos);
            return ((Utilities.URShift((Num - (I != 0 ? DecTab[I - 1] : 0)), (16 - StartPos))) + PosTab[StartPos]);
        }
