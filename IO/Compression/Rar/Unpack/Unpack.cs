...
using IExtendFramework.IO.Compression.Rar.Unpack.decode;
using IExtendFramework.IO.Compression.Rar.Unpack.PPM;
using IExtendFramework.IO.Compression.Rar.Unpack.VM;
    ...
    internal sealed class Unpack : Unpack20
    {
        ...
        */
                ...
                if (Number >= 271)
                {
                    ...
                    if ((Bits = DBits[DistNumber]) > 0)
                    {
                        if (DistNumber > 9)
                        {
                            if (Bits > 4)
                            {
                                Distance += ((Utilities.URShift(GetBits(), (20 - Bits))) << 4);
                                AddBits(Bits - 4);
                            }
        ...
        protected internal override void unpInitData(bool solid)
        {
            ...
            {
                tablesRead = false;
                Utilities.Fill(oldDist, 0); // memset(oldDist,0,sizeof(OldDist));

                oldDistPtr = 0;
                lastDist = 0;
                lastLength = 0;

                Utilities.Fill(unpOldTable, (byte)0); // memset(UnpOldTable,0,sizeof(UnpOldTable));
        ...
        private bool readTables()
        {
            ...
            {
                int length = (Utilities.URShift(GetBits(), 12)) & 0xFF;
                AddBits(4);
                if (length == 15)
                {
                    int zeroCount = (Utilities.URShift(GetBits(), 12)) & 0xFF;
                    AddBits(4);
                    if (zeroCount == 0)
                    {
                        bitLength[i] = 15;
            ...
            UnpackUtilities.makeDecodeTables(bitLength, 0, BD, Compress.BC);
            ...
            UnpackUtilities.makeDecodeTables(table, 0, LD, Compress.NC);
            UnpackUtilities.makeDecodeTables(table, Compress.NC, DD, Compress.DC);
            UnpackUtilities.makeDecodeTables(table, Compress.NC + Compress.DC, LDD, Compress.LDC);
            UnpackUtilities.makeDecodeTables(table, Compress.NC + Compress.DC + Compress.LDC, RD, Compress.RC);
