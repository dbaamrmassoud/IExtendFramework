...
namespace IExtendFramework.Controls
{
	...
	public class ProgressBar : System.Windows.Forms.UserControl
	{
		...
		protected override void OnPaint(PaintEventArgs e)
		{
			...
			if (Shape == eShape.Text) {
				if (ShapeTextRotate != eRotateText.None) {
					Matrix mtrx = new Matrix();
					mtrx.Rotate(GetRotateAngle(ShapeTextRotate));
					_with2.Path.Transform(mtrx);
				}

				mRegion = new Region(_with2.Path);
		...
		public ProgressBarPath GetPath(Rectangle PathRect, bool IsBorder, short ShowDotBorder = 0)
		{
			...
			var _with4 = pPath;
			_with4.Path = new GraphicsPath();
			switch (Shape) {
				case eShape.Rectangle:
					if (IsBorder) {
						_with4.Rect = new Rectangle(PathRect.X, PathRect.Y, PathRect.Width - 1, PathRect.Height - 1);
					} else {
						_with4.Rect = new Rectangle(PathRect.X, PathRect.Y, PathRect.Width - ShowDotBorder, PathRect.Height - ShowDotBorder);
					}
					if (CornersApply == eCornersApply.Bar) {
						_with4.Path.AddRectangle(_with4.Rect);
					} else {
						_with4.Path = GetRoundedRectPath(_with4.Rect, Corners);
					}

					break;
				case eShape.Ellipse:
					if (IsBorder) {
						_with4.Rect = new Rectangle(1, 1, PathRect.Width - 2, PathRect.Height - 2);
					} else {
						_with4.Rect = new Rectangle(PathRect.X + ShowDotBorder, PathRect.Y + ShowDotBorder, PathRect.Width - ShowDotBorder * 2, PathRect.Height - ShowDotBorder * 2);
					}
					_with4.Path.AddEllipse(_with4.Rect);

					break;
				case eShape.TriangleLeft:
					_with4.Rect = new Rectangle(PathRect.X, PathRect.Y, PathRect.Width, PathRect.Height);
					Point[] myArray = null;
					if (IsBorder) {
						myArray = new Point[] {
							new Point(_with4.Rect.Left, Convert.ToInt32(_with4.Rect.Height / 2)),
							new Point(_with4.Rect.Right, 1),
							new Point(_with4.Rect.Right, _with4.Rect.Bottom - 1)
						};
					} else {
						myArray = new Point[] {
							new Point(_with4.Rect.Left + ShowDotBorder, Convert.ToInt32(_with4.Rect.Height / 2)),
							new Point(Convert.ToInt32(_with4.Rect.Right - ShowDotBorder / 2), 0 + ShowDotBorder),
							new Point(Convert.ToInt32(_with4.Rect.Right - ShowDotBorder / 2), _with4.Rect.Bottom - ShowDotBorder)
						};
					}
					_with4.Path.AddPolygon(myArray);
				...
				case eShape.TriangleRight:
					...
					if (IsBorder) {
						myArray = new Point[] {
							new Point(0, 1),
							new Point(0, _with4.Rect.Bottom - 1),
							new Point(_with4.Rect.Right, Convert.ToInt32(_with4.Rect.Height / 2))
						};
					} else {
						myArray = new Point[] {
							new Point(0, 0 + ShowDotBorder),
							new Point(0, _with4.Rect.Bottom - ShowDotBorder),
							new Point(_with4.Rect.Right - ShowDotBorder, Convert.ToInt32(_with4.Rect.Height / 2))
						};
					}
					_with4.Path.AddPolygon(myArray);
				...
				case eShape.TriangleUp:
					...
					if (IsBorder) {
						myArray = new Point[] {
							new Point(Convert.ToInt32(_with4.Rect.Width / 2), _with4.Rect.Top),
							new Point(_with4.Rect.Left + 2, _with4.Rect.Bottom - 1),
							new Point(_with4.Rect.Right - 2, _with4.Rect.Bottom - 1)
						};
					} else {
						myArray = new Point[] {
							new Point(Convert.ToInt32(_with4.Rect.Width / 2), Convert.ToInt32(_with4.Rect.Top + ShowDotBorder / 2)),
							new Point(_with4.Rect.Left + ShowDotBorder * 2, _with4.Rect.Bottom - ShowDotBorder),
							new Point(_with4.Rect.Right - ShowDotBorder * 2, _with4.Rect.Bottom - ShowDotBorder)
						};
					}
					_with4.Path.AddPolygon(myArray);
				...
				case eShape.TriangleDown:
					...
					if (IsBorder) {
						myArray = new Point[] {
							new Point(Convert.ToInt32(_with4.Rect.Width / 2), _with4.Rect.Bottom),
							new Point(_with4.Rect.Left + 1, 0),
							new Point(_with4.Rect.Right - 1, 0)
						};
					} else {
						myArray = new Point[] {
							new Point(Convert.ToInt32(_with4.Rect.Width / 2), Convert.ToInt32(_with4.Rect.Bottom - ShowDotBorder / 2)),
							new Point(_with4.Rect.Left + ShowDotBorder, 0),
							new Point(_with4.Rect.Right - ShowDotBorder, 0)
						};
					}
					_with4.Path.AddPolygon(myArray);
				...
				case eShape.Text:
					...
					if (IsBorder) {
						...
						_with4.Rect = new Rectangle(PathRect.X + 1, PathRect.Y + 1, Convert.ToInt32(PathRect.Width - 2 - ShowDotBorder / 2), PathRect.Height - 2 - ShowDotBorder);
					}
					...
					_with4.Path.AddString(ShapeText, ShapeTextFont.FontFamily, Convert.ToInt32(FontStyle.Bold), _with4.Rect.Height, new PointF(0, 0), sf);
