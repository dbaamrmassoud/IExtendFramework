...
namespace IExtendFramework.IO.Compression.Rar.Unpack.VM
{
    internal class RarVM : BitInput
    {
        ...
        //    Mem.set_Renamed(offset + 1, Byte.valueOf((sbyte) ((Utilities.URShift(value_Renamed, 8)) & 0xff)));
        //    Mem.set_Renamed(offset + 2, Byte.valueOf((sbyte) ((Utilities.URShift(value_Renamed, 16)) & 0xff)));
        //    Mem.set_Renamed(offset + 3, Byte.valueOf((sbyte) ((Utilities.URShift(value_Renamed, 24)) & 0xff)));
        ...
        private int GetValue(bool byteMode, byte[] mem, int offset)
        {
            ...
            else
            {
                if (IsVMMem(mem))
                {
                    return Utilities.readIntLittleEndian(mem, offset);
                }
                else
                {
                    return Utilities.readIntBigEndian(mem, offset);
                }
            }
        }
        ...
        private void SetValue(bool byteMode, byte[] mem, int offset, int value)
        {
            ...
            else
            {
                if (IsVMMem(mem))
                {
                    Utilities.WriteLittleEndian(mem, offset, value);
                ...
                {
                    Utilities.writeIntBigEndian(mem, offset, value);
        ...
        internal void SetLowEndianValue(List<byte> mem, int offset, int value)
        {
            ...
            mem[offset + 1] = (byte)(Utilities.URShift(value, 8) & 0xff);
            mem[offset + 2] = (byte)(Utilities.URShift(value, 16) & 0xff);
            mem[offset + 3] = (byte)(Utilities.URShift(value, 24) & 0xff);
        }
        ...
        private int GetOperand(VMPreparedOperand cmdOp)
        {
            ...
            if (cmdOp.Type == VMOpType.VM_OPREGMEM)
            {
                ...
                ret = Utilities.readIntLittleEndian(Mem, pos);
            }
            else
            {
        ...
        private void filterItanium_SetBits(int curPos, int bitField, int bitPos, int bitCount)
        {
            int inAddr = bitPos / 8;
            int inBit = bitPos & 7;
            int andMask = Utilities.URShift(unchecked((int)0xffffffff), (32 - bitCount));
            ...
            for (int i = 0; i < 4; i++)
            {
                ...
                andMask = (Utilities.URShift(andMask, 8)) | unchecked((int)0xff000000);
        ...
        private int filterItanium_GetBits(int curPos, int bitPos, int bitCount)
        {
            ...
            bitField = Utilities.URShift(bitField, inBit);
            return (bitField & (Utilities.URShift(unchecked((int)0xffffffff), (32 - bitCount))));
        }
